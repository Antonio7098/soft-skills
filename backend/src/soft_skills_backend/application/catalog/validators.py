...
from soft_skills_backend.application.catalog.scenarios.commands import ScenarioCreateCommand
...
def require_existing_skills(session: Session, skill_slugs: list[str]) -> None:
    existing = {
        record.slug for record in session.query(SkillRecord).filter(SkillRecord.slug.in_(skill_slugs)).all()
    }
