...
from soft_skills_backend.api.dependencies import get_identity_service, require_actor
from soft_skills_backend.api.schemas import ApiEnvelope, ok_response
from soft_skills_backend.application.identity import UpdateProfileCommand, UserView
...
async def update_profile(
    request: Request, command: UpdateProfileCommand
) -> ApiEnvelope[UserView]:
    actor = require_actor(request)
    service = get_identity_service(request)
