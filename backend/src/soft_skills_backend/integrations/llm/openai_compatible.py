        ...
        for attempt in range(self._settings.provider_max_retries + 1):
            ...
            try:
                async with asyncio.timeout(self._settings.smoke_timeout_seconds):
                    async with httpx.AsyncClient(timeout=self._settings.smoke_timeout_seconds) as client:
                ...
                latency_ms = int((perf_counter() - start) * 1000)
                timeout_error = (
                    "Provider completion request exceeded the configured timeout budget"
                )
                await self._provider_call_logger.log_call_end(
                    call_id,
                    success=False,
                ...
                if attempt < self._settings.provider_max_retries:
                    await asyncio.sleep(self._settings.provider_retry_backoff_seconds * (attempt + 1))
                    continue
                raise provider_error(
                    "Provider completion request failed",
