"""Prompt planning helpers for catalog generation workflows."""
...
async def generate_collection_blueprint(
    *,
    ctx: Any,
    ...
    if structured_command is not None:
        hardened_notes = sanitize_text(json.dumps(structured_command.realism_notes))
        ...
        operation = "catalog_structured_blueprint_generation"
        prompt_version = config.structured_prompt_version
    else:
        ...
        operation = "catalog_chat_blueprint_generation"
        prompt_version = config.chat_prompt_version
...
async def generate_prompt_item_plan_batch(
    *,
    ctx: Any,
    prompt_library: PromptLibrary,
    config: CatalogGenerationRuntimeConfig,
    ...
    if structured_command is not None:
        rendered_prompt = prompt_library.render(
            config.prompt_item_structured_prompt_name,
            ...
            variables={
                **common_variables,
                ...
                "requested_counts": json.dumps(structured_command.counts.model_dump(mode="json"), sort_keys=True),
        ...
        operation = "catalog_prompt_item_structured_planning"
    else:
        assert chat_command is not None
        try:
            user_message, _ = prompt_security_policy.build_user_message(chat_command.prompt)
        ...
        rendered_prompt = prompt_library.render(
            config.prompt_item_chat_prompt_name,
        ...
        operation = "catalog_prompt_item_chat_planning"
    try:
        return await prompt_item_plan_output.generate(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Plan realistic SoftSkills prompt items for an existing collection. Return JSON only.",
                },
                {"role": "user", "content": rendered_prompt.content},
            ],
            call_context=ProviderCallContext(
