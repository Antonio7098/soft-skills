...
from soft_skills_backend.modules.admin.contracts.unified_audit import (
    PaginatedUnifiedAuditView,
    UnifiedAuditEntryView,
)
from soft_skills_backend.modules.admin.contracts.unified_audit import (
    PaginatedUnifiedAuditView,
    UnifiedAuditEntryView,
...
class AdminService:
    ...
    def list_unified_audit_log(
        ...
        *,
        ...
        entries: list[UnifiedAuditEntryView] = []

        if source is None or source == "workflow_event":
            wf_records = self._workflow_events.list_(
                ...
                from_date=from_date,
                to_date=to_date,
                sort_by=sort_by if sort_by in self._workflow_events.VALID_SORT_FIELDS else None,
        ...
        if source is None or source == "pipeline_run":
            with self._session_factory() as session:
                pr_query = session.query(PipelineRunRecord)
                if event_type:
                    pr_query = pr_query.filter(PipelineRunRecord.pipeline_name == event_type)
                if user_id:
                    pr_query = pr_query.filter(PipelineRunRecord.user_id == user_id)
                if trace_id:
                    pr_query = pr_query.filter(PipelineRunRecord.trace_id == trace_id)
                if request_id:
                    pr_query = pr_query.filter(PipelineRunRecord.request_id == request_id)
                if from_date:
                    pr_query = pr_query.filter(PipelineRunRecord.started_at >= from_date)
                if to_date:
                    pr_query = pr_query.filter(PipelineRunRecord.started_at <= to_date)
                if sort_by == "occurred_at" or sort_by is None:
                    col = PipelineRunRecord.started_at
                    pr_query = pr_query.order_by(col.desc() if sort_order != "asc" else col.asc())
                elif sort_by == "event_type":
                    col = PipelineRunRecord.pipeline_name
                    pr_query = pr_query.order_by(col.desc() if sort_order != "asc" else col.asc())
                else:
                    pr_query = pr_query.order_by(PipelineRunRecord.started_at.desc())
                pr_records = pr_query.offset(offset).limit(limit).all()
                for r in pr_records:
                    entries.append(
                        UnifiedAuditEntryView(
                            id=r.pipeline_run_id,
                            source="pipeline_run",
                            event_type=r.pipeline_name,
                            user_id=r.user_id,
                            trace_id=r.trace_id,
                            request_id=r.request_id,
                            error_code=r.error,
                            occurred_at=r.started_at.isoformat(),
                            payload={
                                "status": r.status,
                                "failed_stage": r.failed_stage,
                                "stage_results": r.stage_results or {},
                            },
                        )
                    )

        if source is None or source == "provider_call":
            with self._session_factory() as session:
                pc_query = session.query(ProviderCallRecord)
                if event_type:
                    pc_query = pc_query.filter(ProviderCallRecord.operation == event_type)
                if trace_id:
                    pc_query = pc_query.filter(ProviderCallRecord.trace_id == trace_id)
                if request_id:
                    pc_query = pc_query.filter(ProviderCallRecord.request_id == request_id)
                if error_code:
                    pc_query = pc_query.filter(ProviderCallRecord.error.isnot(None))
                if from_date:
                    pc_query = pc_query.filter(ProviderCallRecord.created_at >= from_date)
                if to_date:
                    pc_query = pc_query.filter(ProviderCallRecord.created_at <= to_date)
                if sort_by == "occurred_at" or sort_by is None:
                    col = ProviderCallRecord.created_at
                    pc_query = pc_query.order_by(col.desc() if sort_order != "asc" else col.asc())
                elif sort_by == "event_type":
                    col = ProviderCallRecord.operation
                    pc_query = pc_query.order_by(col.desc() if sort_order != "asc" else col.asc())
                else:
                    pc_query = pc_query.order_by(ProviderCallRecord.created_at.desc())
                pc_records = pc_query.offset(offset).limit(limit).all()
                for r in pc_records:
                    entries.append(
                        UnifiedAuditEntryView(
                            id=r.call_id,
        ...
        total = len(entries)
        return PaginatedUnifiedAuditView(
            items=entries[:limit],
            total=total,
            offset=offset,
            limit=limit,
