"""Guardrails for admin-agent SQL."""

from __future__ import annotations

import re
from dataclasses import dataclass

from soft_skills_backend.modules.admin_agent.contracts.commands import (
    AdminAgentScalar,
    QueryAdminDataCommand,
)
from soft_skills_backend.modules.admin_agent.domain.schema_registry import AdminAgentSchemaRegistry
from soft_skills_backend.shared.errors import validation_error

_RELATION_PATTERN = re.compile(
    r"\b(?P<keyword>from|join)\s+"
    r"(?P<view>admin_agent_[a-zA-Z0-9_]+_v)"
    r"(?:\s+(?:as\s+)?"
    r"(?P<alias>(?!where\b|group\b|order\b|limit\b|join\b|on\b|having\b)[a-zA-Z_][a-zA-Z0-9_]*))?",
    re.IGNORECASE,
)
_DENIED_TOKEN_PATTERN = re.compile(
    r"\b(insert|update|delete|alter|drop|truncate|attach|detach|pragma|create|replace|merge|call|copy|vacuum)\b",
    re.IGNORECASE,
)
_SET_OPERATION_PATTERN = re.compile(r"\b(union|intersect|except)\b", re.IGNORECASE)
_COUNT_STAR_PATTERN = re.compile(r"^\s*count\s*\(\s*\*\s*\)\s*(?:as\s+\w+)?\s*$", re.IGNORECASE)


@dataclass(frozen=True, slots=True)
class GuardedAdminQuery:
    sql: str
    scoped_sql: str
    params: dict[str, AdminAgentScalar]
    source_views: tuple[str, ...]
    row_cap_applied: bool


class AdminAgentSqlGuard:
    """Validate and scope admin-agent read-only SQL."""

    def __init__(
        self,
        *,
        schema_registry: AdminAgentSchemaRegistry,
        row_limit: int,
    ) -> None:
        self._schema_registry = schema_registry
        self._row_limit = row_limit

    def validate_and_scope(self, command: QueryAdminDataCommand) -> GuardedAdminQuery:
        sql = self._normalize_sql(command.sql)
        self._reject_disallowed_sql(sql)
        source_views = self._extract_source_views(sql)
        self._reject_select_wildcards(sql)
        scoped_sql = self._scope_sql(sql)
        return GuardedAdminQuery(
            sql=sql,
            scoped_sql=(
                "SELECT * FROM "
                f"({scoped_sql}) AS admin_agent_result "
                "LIMIT :_admin_agent_row_limit"
            ),
            params=dict(command.params),
            source_views=source_views,
            row_cap_applied=True,
        )

    def _normalize_sql(self, sql: str) -> str:
        normalized = sql.strip()
        if normalized.endswith(";"):
            normalized = normalized[:-1].rstrip()
        if not normalized:
            raise validation_error(
                "Admin agent SQL must not be blank",
                code="SS-VALIDATION-301",
            )
        return normalized

    def _reject_disallowed_sql(self, sql: str) -> None:
        lowered = sql.lower()
        if not lowered.startswith("select "):
            raise validation_error(
                "Admin agent accepts SELECT queries only",
                code="SS-VALIDATION-302",
            )
        if "--" in sql or "/*" in sql or "*/" in sql or ";" in sql:
            raise validation_error(
                "Admin agent SQL comments and multi-statement syntax are not allowed",
                code="SS-VALIDATION-303",
            )
        if "(select" in lowered or _SET_OPERATION_PATTERN.search(sql):
            raise validation_error(
                "Admin agent SQL must not use subqueries or set operations",
                code="SS-VALIDATION-304",
            )
        if _DENIED_TOKEN_PATTERN.search(sql):
            raise validation_error(
                "Admin agent SQL contained a denied statement",
                code="SS-VALIDATION-305",
            )

    def _extract_source_views(self, sql: str) -> tuple[str, ...]:
        views: list[str] = []
        for match in _RELATION_PATTERN.finditer(sql):
            view_name = match.group("view")
            if not self._schema_registry.has_view(view_name):
                raise validation_error(
                    "Admin agent SQL referenced a disallowed target",
                    code="SS-VALIDATION-306",
                    details={"target": view_name},
                )
            views.append(view_name)
        if not views:
            raise validation_error(
                "Admin agent SQL must query an allowlisted admin view",
                code="SS-VALIDATION-307",
            )
        return tuple(dict.fromkeys(views))

    def _reject_select_wildcards(self, sql: str) -> None:
        select_clause = _extract_select_clause(sql)
        for item in _split_top_level_csv(select_clause):
            if "*" not in item:
                continue
            if _COUNT_STAR_PATTERN.match(item):
                continue
            raise validation_error(
                "Admin agent SQL must project explicit columns",
                code="SS-VALIDATION-308",
                details={"select_item": item.strip()},
            )

    def _scope_sql(self, sql: str) -> str:
        def replace(match: re.Match[str]) -> str:
            keyword = match.group("keyword").upper()
            view_name = match.group("view")
            alias = match.group("alias") or view_name
            return (
                f"{keyword} "
                f"(SELECT * FROM {view_name} WHERE organisation_id = :organisation_id) "
                f"AS {alias}"
            )

        return _RELATION_PATTERN.sub(replace, sql)


def _extract_select_clause(sql: str) -> str:
    lowered = sql.lower()
    depth = 0
    for index, char in enumerate(sql):
        if char == "(":
            depth += 1
        elif char == ")":
            depth = max(0, depth - 1)
        elif depth == 0 and lowered[index : index + 5] == " from":
            return sql[6:index]
    raise validation_error(
        "Admin agent SQL must include a FROM clause",
        code="SS-VALIDATION-309",
    )


def _split_top_level_csv(value: str) -> list[str]:
    parts: list[str] = []
    current: list[str] = []
    depth = 0
    for char in value:
        if char == "(":
            depth += 1
        elif char == ")":
            depth = max(0, depth - 1)
        if char == "," and depth == 0:
            parts.append("".join(current).strip())
            current = []
            continue
        current.append(char)
    if current:
        parts.append("".join(current).strip())
    return [part for part in parts if part]
