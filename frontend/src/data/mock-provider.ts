...
export const mockDataProvider: DataProvider = {
  ...
  async listUnifiedAuditLog(params?: {
    source?: string;
    event_type?: string;
    user_id?: string;
    trace_id?: string;
    ...
    if (params?.source === undefined || params?.source === 'workflow_event') {
      ...
      if (params?.trace_id) filtered = filtered.filter((e) => e.trace_id === params.trace_id);
      if (params?.request_id) filtered = filtered.filter((e) => e.request_id === params.request_id);
      if (params?.error_code) filtered = filtered.filter((e) => e.error_code === params.error_code);
      if (params?.from_date) { const d = new Date(params.from_date); filtered = filtered.filter((e) => new Date(e.occurred_at) >= d); }
      if (params?.to_date) { const d = new Date(params.to_date); filtered = filtered.filter((e) => new Date(e.occurred_at) <= d); }
      if (params?.search) { try { const r = new RegExp(params.search, 'i'); filtered = filtered.filter((e) => r.test(e.event_type) || r.test(e.trace_id || '') || r.test(e.user_id || '')); } catch { filtered = []; } }
      filtered.sort((a, b) => sortOrderCmp(a.occurred_at, b.occurred_at, params?.sort_order));
      for (const e of filtered.slice(0, params?.limit ?? 50)) {
        entries.push({ id: e.event_id, source: 'workflow_event', event_type: e.event_type, user_id: e.user_id, trace_id: e.trace_id, workflow_id: e.workflow_id, request_id: e.request_id, error_code: e.error_code, occurred_at: e.occurred_at, payload: e.payload || {} });
      }
    }

    const offset = params?.offset ?? 0;
    const limit = params?.limit ?? 50;
    entries.sort((a, b) => sortOrderCmp(a.occurred_at, b.occurred_at, params?.sort_order));
